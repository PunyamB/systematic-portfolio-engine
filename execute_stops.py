...
#
# The stop exits file is cleared after execution to prevent
# duplicate orders on subsequent main.py runs.
...
from data.storage import load_portfolio, save_stop_exits
...
def _find_stop_exits_file(run_date: date) -> Path:
    """Finds the stop exits CSV for the given date."""
...
def _update_executed_weights(stop_tickers: list) -> None:
    ...
    This updates the drift detection anchor so stopped positions
    don't cause false drift triggers.
    """
...
def _clear_stop_exits_file(filepath: Path) -> None:
    """Removes the stop exits file after execution to prevent re-processing."""
    if filepath.exists():
        filepath.unlink()
...
def run_stop_execution(run_date: date = None) -> dict:
    """
    Reads stop exits file, submits sell orders, confirms fills,
    updates portfolio and executed weights, clears the file.
    """
    if run_date is None:
        run_date = date.today()
    ...
    filled_tickers = fills[fills["filled"] == True]["ticker"].tolist() if not fills.empty and "filled" in fills.columns else []
    ...
    # ----------------------------------------------------------
    # STEP 7 -- Clear the stop exits file
    # ----------------------------------------------------------
    _clear_stop_exits_file(stop_file)
